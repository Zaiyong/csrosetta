...
def delete_pyc_files():
    if 'csrosettaDir' not in environ:
        ...
        exit()
    for r,d,f in os.walk(environ['csrosettaDir']):
	for files in f:
...
./integration.py ligand_dock_7cpa    # only run the "ligand_dock_7cpa" test
    ...
    #   metavar="PLACE",
    # )
    parser.add_option("--toolbox_home",
      #default=path.join( path.expanduser("~"), "mini"),
      default= path.join( path.dirname( path.dirname( path.dirname(path.abspath(sys.argv[0])) ) ), 'com_alias'),
      help="Directory where csrosetta3 toolbox is found (default: ../../)",
    )
    ...
    parser.add_option("--log",
      default=None,
      help="Save results to specified file, default is None",
    )
    parser.add_option("--cur_commit",
      default=None,
      help='current commit',
    )
    parser.add_option("--ex_commit",
      default=None,
      help='fomer commit',
    )
    ...
    options.toolbox_home = path.abspath( options.toolbox_home )
    print 'Using Toolbox dir at:', options.toolbox_home
    ...
    if not path.isdir("tests"):
        print "You must run this script from csrosetta3/tests/integration/"
        return 2
                ...
                if options.fulldiff:
                    ...
                    for line in lines :
                        ...
                        if len(cols) < 4 or not( cols[2] == "and" ) :
                            ...
                            msg += "    nonempty diff %s %s\n" % ( cols[1], cols[3].strip() )
                            if options.cur_commit and options.ex_commit:
                                dir1=cols[1].replace('ref/','ref_'+options.ex_commit+'/',1)
                                dir2=cols[3].strip().replace('new/','ref_'+options.cur_commit+'/',1)
                                full_log_msg += "     nonempty diff %s %s\n" %  ( dir1, dir2 )
                            else:
                                full_log_msg += "     nonempty diff %s %s\n" %  ( cols[1], cols[3].strip() )
            full_log_msg += '\n'
            ...
            while True:
                ...
                try: # Actually catch exception and ignore it.  Python 2.4 can't use "except" and "finally" together.
                    ...
                    try: # Make sure job is marked done even if we throw an exception
                        ...
                        minidir = self.opts.toolbox_home
                        bin = minidir
