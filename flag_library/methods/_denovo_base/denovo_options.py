...
if 'group' in locals():
	...
	group.add_argument("-disulfide_bonds", nargs="*", help="specify pairs of residues that are bound in S-S bond", default=None )
...
class DenovoBaseMethod(automatic_setup.BasicMethod):
	def __init__(self,name,path):
		...
		self.non_file_options.append('disulfide_bonds')
...
# 		return (retf3, retf9)
	...
	def add_restraint_scores(self):
		fl = self.file_library
		fl.add("flags","flags_denovo","@flags_nmr_patches")

	def make_disulf_files(self, run, setup, disulf_list, flags ):
		if len(disulf_list) % 2 != 0:
			raise InconsistentInput( "need to define pairs of disulfide bonded residues" )
		disulf_pairs=[]
		for i in range( 0, len(disulf_list), 2 ):
			disulf_pairs.append( (int(disulf_list[i]), int(disulf_list[i+1])) )

#AtomPair CB     12 CB     24 BOUNDED 0.0 5.0 1.0 disulfide
#AtomPair CB     17 CB     31 BOUNDED 0.0 5.0 1.0 disulfide
#AtomPair CB      3 CB     18 BOUNDED 0.0 5.0 1.0 disulfide
#AtomPair CB     37 CB     41 BOUNDED 0.0 5.0 1.0 disulfide
		disulf_cen = setup.create_file( "centroid_disulfide_constraints.cst", subdir='' )
		disulf_cen_file = open( setup.abspath( disulf_cen ), 'w' )

		disulf_conn = setup.create_file("disulfide_connectivity.cst", subdir='' )
		disulf_conn_file = open( setup.abspath( disulf_conn ), 'w' )

		for i,j in disulf_pairs:
			disulf_cen_file.write('AtomPair CB %7d CB %7d BOUNDED 1.0 5.0 1.0 disulfide\n'%(i,j))
			disulf_conn_file.write('%7d %7d\n'%(i,j))
		disulf_conn_file.close()
		disulf_cen_file.close()
		flags.write( '-fix_disulf disulfide_connectivity.cst\n' )
		run.add_file_to_rundir( setup, 'disulfide_connectivity.cst' )
		run.add_file_to_rundir( setup, 'centroid_disulfide_constraints.cst' )
		self.broker(setup).add_cst_claimer( setup.cm_path( "centroid_disulfide_constraints.cst" ), fullatom=False, centroid=True, skip=0, combine=False)
...
#        from library import *
		...
		if args.disulfide_bonds:
			self.make_disulf_files( run, setup, args.disulfide_bonds, flags )
		...
		if args.centroid_stage_restraints:
			...
			for cst in args.centroid_stage_restraints:
				if not library.cst_is_centroid( cst ):
...
#			flags.write("@flags_nmr_patches\n")
   ...
   def setup_file_library( self ):
		...
		if args.disulfide_bonds:
			fl.override("flags", "flags_nmr_patches", "-abinitio::stage4_patch disulf_and_nmr_patch" )
			self.restraints_in_score=True
