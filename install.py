...
parser.add_argument('-platform', choices=['linux','macos'], default=None, help='specify the platform [linux,macos]' )
parser.add_argument('-build', choices=['default','mpi'], default='default', help='specify which executables from Rosetta to use [default,mpi]')
...
parser.add_argument('-blast_platform', default=None, help='platform string to select appropriate blast executable when downloading')
...
def check_db_path(db_path):
	print 'check_db: ', db_path
	if path.exists('%s/chemical/atom_type_sets/fa_standard/atom_properties.txt'%db_path):
		return db_path
	print 'check_db:  %s/database'%db_path
	if path.exists('%s/database/chemical/atom_type_sets/fa_standard/atom_properties.txt'%db_path):
		return '%s/database'%db_path
	raise DBNotFound

def check_rosetta_version( rosetta_source, quiet=False ):
	rosetta_test_executable='minirosetta.default.linuxgccrelease'
	rosetta_source=rosetta_source.replace('bin/','')
	if path.exists(rosetta_source+'/bin') and path.exists(rosetta_source+'/bin/%s'%rosetta_test_executable):
		if not quiet: print 'AUTO-DETECTION: check rosetta version in %s via %s'%(rosetta_source, rosetta_test_executable )
		cmd='%s/bin/%s -help | grep iterative:normalize | wc -l'%(rosetta_source, rosetta_test_executable)
...
def print_final_msgs( final_msgs ):
...
def silent_wget( link, dir, verbose=False,options="-N -c",progress=True ):
	...
	def progressbar(skipped,current,full):
		if full<=0:
			return 'no progress bar, full length unknown',0
		N=100
...
#determine plastform. If not given explicitly on cmd-line use sys.platform
...
if not platform:
	if 'linux' in sys.platform:
		...
		platform='macos'
	if platform:
		print 'AUTO-DETECTED: Running on %(platform)s...'%locals()

#for blast the linux platform string is x64-linux, for mac it is just mac.
build=args.build
blast_platform='x64-linux'
if not platform in blast_platform:
	blast_platform=platform
if args.blast_platform:
	blast_platform=args.blast_platform

#get talos-directory, if it is a relastive path make it absolute
talos_dir=None
if args.talos_dir:
...
def find_rosetta( rosetta_arg ):
	#look for rosetta, make sure to have a correct version
	#if cmd-line use that, but make path absolute
	#not given look for rosetta executable in PATH
	#for the check we assume that rosetta exectuable live in a directory ending on .../bin
	#we call check_rosetta_version on every path in PATH environment variable
	#if multiple versions of rosetta are found, we take the last one in the PATH (shouldn't it be the other way around).
	rosetta_dir=None
	rosetta_bin=None
	if rosetta_arg:
		rosetta_dir=rosetta_arg
	...
	if not rosetta_dir:
		...
		for x in rosetta_paths:
			if check_rosetta_version( x.replace('/bin','') ):
				rosetta_dir=x.replace('/source/bin','')
				print 'AUTO-DETECTION: found rosetta in $PATH as %s'%rosetta_dir
				rosetta_bin=x
	else: #rosetta hint was given on cmd-line. let's see what we can do with that.
		head=rosetta_dir
		tail=None
		while len(head)>1:
#			print 'check ', head
			head,tail = path.split(head)
			if check_rosetta_version( head ):
				break
		if not len(head)>1:
			add=['source','main/source','rosetta/main/source','Rosetta/main/source']
			for dir in add:
	#			print 'check', rosetta_dir+'/'+dir
				if check_rosetta_version( rosetta_dir + '/'+dir ):
					rosetta_dir = rosetta_dir + '/' + dir
					break
		else:
			rosetta_dir = head
	#rosetta was not found in PATH, try some standard locations
	if not rosetta_dir:
		home= os.path.realpath(os.environ['HOME'])
		print 'AUTO-DETECTION: look in $HOME/rosetta'
		if check_rosetta_version( home+'/rosetta/main/source' ):
			rosetta_dir=home+'/rosetta/main'
	else:
		rosetta_dir=rosetta_dir.replace('/source/bin','').replace('/source','').rstrip('/')

	#final paranoia check: quiet because it is probably double
	if rosetta_dir:
		if not check_rosetta_version( rosetta_dir+'/source', quiet=True ):
			print 'AUTO-DETECTION FAILED: best match was in %s -- but this is wrong Rosetta version'%rosetta_dir
			rosetta_dir=None
			rosetta_bin=None
		else:
			if not rosetta_bin:
				rosetta_bin='%(rosetta_dir)s/source/bin'%locals()

	#final paranoia check for rosetta_bin
	if rosetta_bin:
		if not check_rosetta_version( rosetta_bin.replace('/bin',''), quiet=True ):
			print 'AUTO-DETECTION FAILED: best match was in %s -- but this has wrong Rosetta version'%rosetta_bin
			rosetta_bin=None

	if not rosetta_arg and rosetta_bin:
		print 'AUTO_DETECTED: ROSETTA bin directory at '+rosetta_bin

	return rosetta_bin, rosetta_dir

def find_rosetta_db( args_rosetta_database, rosetta_bin, final_msgs ):
	rosetta_db_path=None
	db_source=None
	if args_rosetta_database:
		try:
			rosetta_db_path=check_db_path(args_rosetta_database)
			db_source='cmd-line'
		except DBNotFound:
			exit('Cannot find the ROSETTA Database directory at the provided path %s'%args_rosetta_database)
	elif rosetta_bin:
	...
	if rosetta_db_path and not args_rosetta_database:
	...
	return rosetta_db_path

def check_dunbrack_library( rosetta_db, rosetta_bin, build, platform, final_msgs ):
	#run score_jd2 twice on tutorials/input/2jrmA.pdb
	#the second time it should definitely read the dunbrack library from the binary otherwise something is wrong
	model='tutorials/inputs/2jrmA.pdb'
	exe='score_jd2.%(build)s.%(platform)sgccrelease'%locals()
	cmdline='%(rosetta_bin)s/%(exe)s -database %(rosetta_db)s -in:file:s %(model)s -out:levels all:error core.pack.dunbrack:info'%locals()

#call 1st time
	print 'check whether dunbrack library can be loaded from the binary file...'
	subprocess.call(cmdline,shell=True, cwd=os.getcwd())
...
#call 2nd time
	...
	for l in pipe.stdout:
		if 'to load from binary' in l:
			load_from_binary=True
			break

	if not load_from_binary:
		final_msgs.add(('\n[WARNING] Cannot load dunbrack database from binary.\n'+
								   '             This probably means that we cannot write to the database directory.\n'+
									 '             To remedy this run\n'+
									 '                %(cmdline)s\n'+
									 '             as the user who has write access to the database directory\n'+
									 '             If this command fails to execute update Rosetta and Rosetta-database and recompile the %(build)s version')%locals())

	try:
		os.remove('score.sc')
	except OSError:
		pass
	return load_from_binary

#################################################
# main

head,tail=path.split(os.getcwd())
if tail=='com':
	os.chdir('..')


#call find_rosetta and find_rosetta_db
rosetta_bin, rosetta_dir = find_rosetta( args.rosetta )
rosetta_db_path = find_rosetta_db( args.rosetta_database, rosetta_bin, final_msgs )
if not check_dunbrack_library( rosetta_db_path, rosetta_bin, build, platform, final_msgs ):
	print 'Cannot generate binary dunbrack library'
...
export ROSETTA3_BUILD=%(build)s
export ROSETTA3_PLATFORM=%(platform)s
...
#make com_aliases
import subprocess
for ext in ['py']: #removed 'com' as there are no left
...
if not talos_dir:
  #try autodection of TALOS+
	...
	if k != 'NOT_FOUND':
		...
		talos_exec='/bin/TALOS+.%(platform)s'%locals()
		if path.exists(talos_dir+talos_exec):
			print 'AutoDetected: TALOS: %s in environment variable %s'%(talos_dir, k)
		else:
			talos_dir=None

if not sparta_dir:
	keys=['SPARTAP_DIR','NOT_FOUND']
	for k in keys:
		if k in os.environ:
			break
	if k=='NOT_FOUND' and talos_dir:
		sparta_dir=talos_dir.replace('talosplus','spartaplus')
	elif k!='NOT_FOUND':
		sparta_dir=os.path.normpath(os.path.realpath(os.environ[k]))
	if sparta_dir:
		sparta_exec='/bin/SPARTA+.%(platform)s'%locals()
		if path.exists(sparta_dir+sparta_exec):
			print 'AutoDetected: SPARTA+: %s'%(sparta_dir)
		else:
			sparta_dir=None
...
Please provide TALOS+ root directory with option -talos_dir"""
		...
		while attempts>0 and (args.check or not stat_old):
			...
			if stat_old!=stat_new:
				...
				unpack=True
			if not unpack and not path.exists('%(nmrPipe_dir)s/talosplus/bin/TALOS+.%(platform)s'%locals()):
				unpack=True
				print '%(nmrPipe_dir)s/talosplus/bin/TALOS+.%(platform)s not found, unpacking'%locals()
			if unpack:
...
if talos_dir:
	...
	pipe=subprocess.Popen('%(talos_dir)s/bin/TALOS+.%(platform)s'%locals(),shell=True,stderr=subprocess.PIPE)
	for line in pipe.stderr:
		if 'Version' in line:
			print line
			years=['2009','2012']
...
subprocess.call('ln -sf %(localized_talos)s/bin/TALOS+.%(platform)s %(root)s/com_alias/talos+'%locals(), shell=True)
...
if args.blast:
	...
	blast_tgz=glob.glob(root+'/frag_picker/blast-*%s*.tar.gz'%blast_platform)
	if len(blast_tgz)==0:
		print '-'*delim_width
		print 'cannot find tarball for BLAST executable. Attempt to download...\n '
		print 'will download executable with platform-key "%s" (-blast_platform)'%blast_platform
	...
	#	subprocess.call('cd %s/frag_picker;'%root+'wget ftp://ftp.ncbi.nlm.nih.gov/blast/executables/release/LATEST/ -O blast_index.html;'+
		...
		subprocess.call('grep /blast- blast_index.html | grep %s > blast_url'%blast_platform,cwd='%s/frag_picker'%root, shell=True)
	...
	if not path.exists( 'installed_blast_platform' ):
		open('installed_blast_platform','w').write('unknown\n')
	for l in open('installed_blast_platform','r'):
		if len(l.strip()) and not platform in l.strip():
			print 'change of blast platform detected...\nremove old blast directory...'
			try:
				shutil.rmtree(os.readlink('blast'))
				os.remove('blast')
			except OSError:
				pass

  if not path.exists( 'blast/bin/blastpgp' ):
		print 'unpack blast tar-ball...'
		open('installed_blast_platform','w').write(platform+'\n')
