...
#		print "initialized method %s"%self.name
		...
		self.target_jobs = ['production','test']
...
#			print "flattended: ",val
	...
	def clear_double_file_options(self):
		for opt in self.double_file_options:
			try:
				del self.target_options[opt]
			except KeyError:
				pass
...
#						print "%-50s transfered from previous setup"%(file)
			...
			#		print "F2: ", val
					try:
						...
						raise MethodException( self, "TypeError expected opt %s to be a file. \
                             Maybe option should be added to non_file_options[] ?!"%opt );
