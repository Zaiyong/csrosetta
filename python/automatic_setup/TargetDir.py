...
class TrackChangeList( list ):
	def __init__( self ):
		list.__init__( self )
		self._init_length = len( self )
		self._clean = True
	...
	def is_clean( self ):
		return self._clean and len( self ) == self._init_length
...
#		sfile, subdir=subdir_name(file, subdir, setup_dir)
	...
	def cm_dir( self, levels ):
		...
		print "CM_DIR: ", path.normpath(prefix+'/'+self.dir()).replace('//','/')
	...
	def __del__(self):
		if len(self.files) and not self.files.is_clean():
