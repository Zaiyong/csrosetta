...
class RunGenerator:
	...
	def __init__(self, rundir, flag_lib, platforms, bindir, database=None, extension=".default.linuxgccrelease", subdir_level=1 ):
		...
		self.rerun=False
		if not database:
			raise MissingInput('Need database: either specify ROSETTA3_DB environment variable or use -database option')
		if database[0]!="/":
			db_prefix="../../"
		else:
			db_prefix=""
...
#		print "CMDLINE: ",cmdline
	...
	def add_file_to_rundir( self, setup, file ):
		shutil.copy( setup.abspath(file), self._fn_rundir+"/run/" )
...
#			print "generate job-script: %s"%platform
			...
			job_type=filename.split('.')[0]
			if not job_type in method.target_jobs: continue
...
#			"CM_FASTA" :      target_lib.fn_fasta,
			...
			"CM_TARGETPATH" : setup.target.cm_dir(self.subdir_level+1), #the +1 because we go to /run or /test level down from -dir (for the target).
