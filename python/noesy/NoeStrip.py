...
#			print self._proton.freq_str(), cp.proton_resonance(self.indirect_dim()), self._proton.peak_pmatch( cp, indirect )
	...
	def __str__(self):
		s='Strip(%3s %s |%3s %s )'%(self._proton.name(), self._proton.freq_str(),self._label.name(), self._label.freq_str())
...
#		print 'mid points: ', mid_points
		...
		for freq in mid_points:
			strip_proton=Resonance(freq=freq,error=0,atom=proton.atom())
...
'''
	...
	noestrip_test=NoeStrip.generate_strip(pl,1,Resonance(1,Atom('H',110),2.695,0),Resonance(2,Atom('CB',110),45.226,0))
	print noestrip_test
